...
export default function ProfilePage() {
  ...
  useEffect(() => {
    ...
    supabase
      ...
      .eq("id", user.id)
      .single()
      .then(({ data, error }) => setProfile(data));
    supabase
      .from("profiles_ext")
      .select("*")
      .eq("user_id", user.id)
      ...
      .then(({ data }) => {
        ...
        setConsentDates({
          agb: data?.consent_agb_at || null,
          privacy: data?.consent_privacy_at || null,
          marketing: data?.consent_marketing_at || null,
        });
        setLoading(false);
      });
  }, [user]);
  ...
  async function handleSave(e: React.FormEvent) {
    ...
    await supabase
      ...
      .upsert({
        ...
        consent_agb_at: consents.agb && !consentDates.agb ? new Date().toISOString() : consentDates.agb,
        consent_privacy_at: consents.privacy && !consentDates.privacy ? new Date().toISOString() : consentDates.privacy,
        consent_marketing_at: consents.marketing && !consentDates.marketing ? new Date().toISOString() : consentDates.marketing,
      })
      .eq("user_id", user.id);
  ...
  function handleExtChange(field: string, value: string) {
    setExt({ ...ext, [field]: value });
  }
